...
class TestBatchModel:
    ...
    def test_batch_allows_empty_batch_number(self):
        """Test che batch_number possa essere vuoto (per dati legacy)."""
        # Questo è permesso per compatibilità con dati esistenti
        batch = Batch(
            supplier_id=1,
            product_name='Test',
            batch_number=''  # OK per dati legacy
        )
        assert batch.batch_number == ''
    ...
    def test_batch_vials_validation(self):
        ...
        # Vials remaining > count è permesso (può succedere con adjust_vials)
        batch = Batch(
            supplier_id=1,
            product_name='Test',
            batch_number='BATCH001',
            vials_count=5,
            vials_remaining=10  # Permesso! (correzione errori)
        )
        assert batch.vials_remaining == 10
...
class TestBatchRepository:
    ...
    def test_create_batch_requires_batch_number(self, repo):
        """Test che create() richieda batch_number (per nuovi batch)."""
        batch = Batch(
            supplier_id=1,
            product_name='Test',
            batch_number='',  # Vuoto
        ...
        # create() deve validare che batch_number non sia vuoto
        with pytest.raises(ValueError, match="batch_number obbligatorio"):
            repo.create(batch)
